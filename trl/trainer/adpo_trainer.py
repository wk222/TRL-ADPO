...
RolloutFunc = Callable[[list[str], "ADPOTrainer"], dict[str, Any]]


class ADPOTrainer(BaseTrainer):
    """
    Trainer for Anchored Direct Preference Optimization (ADPO) method.
    
    ADPO extends preference learning with an anchored distribution that provides geometric stability.
    Unlike GRPO's PPO-style clipping, ADPO uses: p_θ(i|S) = softmax((s_i - s_anchor_i) / τ)
    ...
    trainer = ADPOTrainer(
        ...
        args=ADPOConfig(
            ...
            num_generations=8,
            tau=1.0,
            # anchor_update_mode="on_policy" is the default (like GRPO)
            # use "fixed" for standard off-policy ADPO
        ),
    )
    trainer.train()
    ```
    ...
    _name = "ADPO"
    _paper = {
        "title": "Anchored Direct Preference Optimization",
        "id": "2510.18913",
        # docstyle-ignore
    ...
    def _set_signature_columns_if_needed(self):
        ...
        # In ADPOTrainer, we preprocess data, so using the model's signature columns doesn't work.
